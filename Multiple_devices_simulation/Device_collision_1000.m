...
hold on
yyaxis left
plot(nrofdevices,results(:, 1))
ylabel('Nr of collisions or Fails'), ylim([0 1000])
yyaxis right
ylabel('Packet error rate (%)'), ylim([0 100])
plot(nrofdevices,results(:, 2))
legend('number of failed transmissions', 'PER');
grid on,hold off;

saveas(figure(99), sprintf('lora_%d_dev_sf_%d_%d.png',maxnrofdevices, lora_duration(start_channel), lora_duration(end_channel)));
...
%saveas(figure(99), sprintf('lora_%d_dev_sf_%d_%d.fig',maxnrofdevices, lora_duration(start_channel), lora_duration(end_channel)));
